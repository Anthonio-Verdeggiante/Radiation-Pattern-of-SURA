classdef DipoleMatrixApp8 < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function matrix = initializeMatrix(app, powers, frequency, theta)
            ...
            phase_base = 2 * pi * frequency * (dipole_length / c) * rand(12, 4); % Случайная базовая фаза
            for j = 1:4
                ...
                base_matrix(:, j, 1) = sqrt(powers(1)) * amplitude_factor * amplitude_variation(:, j) .* exp(1i * phase_1);
                base_matrix(:, j, 2) = sqrt(powers(1)) * amplitude_factor * amplitude_variation(:, j) .* exp(1i * phase_2);
            end
            ...
            for j = 1:4
                ...
                base_matrix(:, j+4, 1) = sqrt(powers(2)) * amplitude_factor * amplitude_variation(:, j) .* exp(1i * phase_1);
                base_matrix(:, j+4, 2) = sqrt(powers(2)) * amplitude_factor * amplitude_variation(:, j) .* exp(1i * phase_2);
            end
            ...
            phase_base = 2 * pi * frequency * (dipole_length / c) * rand(12, 4);
            for j = 1:4
                % Линейный сдвиг фазы для столбцов 9-12: (j+7) * theta_shift_per_column
                ...
                base_matrix(:, j+8, 1) = sqrt(powers(3)) * amplitude_factor * amplitude_variation(:, j) .* exp(1i * phase_1);
                base_matrix(:, j+8, 2) = sqrt(powers(3)) * amplitude_factor * amplitude_variation(:, j) .* exp(1i * phase_2);
            end
            ...
            function updatePlot(app)
                ...
                % Суммируем комплексные числа двух диполей для каждого элемента
                complex_sum = app.Matrix(:, :, 1) + app.Matrix(:, :, 2);
                % Интенсивность как квадрат модуля результирующей суммы
