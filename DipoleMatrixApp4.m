classdef DipoleMatrixApp4 < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function [psi_base, Fn_psi, theta_x, Fn_theta_x, theta_y, Fn_theta_y, theta_x_3d, theta_y_3d, Fn_3d, x_3d, y_3d, Fn_xy] = computeAmplitudePattern(app, frequency)
            ...
            % 2. Зависимость Fn от theta_x (вдоль оси x, с учётом theta_slider и поправки 34 градуса)
            theta_x = linspace(-pi/2, pi/2, 181); % Углы от -90 до 90 градусов (в радианах)
            I_x = mean(abs(app.Matrix(Nx, :))); % Средняя амплитуда для ряда Nx
            term1_x = 1 - exp(1i * Nx_total * k * d * cos(theta_x + theta_offset - theta_slider - ksi));
            term2_x = 1 - exp(1i * k * d * cos(theta_x + theta_offset - theta_slider - ksi));
            term2_x(abs(term2_x) < eps) = eps; % Избегаем деления на ноль
            Fn_theta_x = abs(I_x * term1_x ./ term2_x);
            Fn_theta_x = Fn_theta_x / max(Fn_theta_x); % Нормируем на максимум
            
            % 3. Зависимость Fn от theta_y (вдоль оси y, с учётом theta_slider и поправки 34 градуса)
            theta_y = linspace(-pi/2, pi/2, 181); % Углы от -90 до 90 градусов (в радианах)
            I_y = mean(abs(app.Matrix(:, Ny))); % Средняя амплитуда для столбца Ny
            term1_y = 1 - exp(1i * Ny_total * k * d * cos(theta_y + theta_offset - theta_slider - ksi));
            term2_y = 1 - exp(1i * k * d * cos(theta_y + theta_offset - theta_slider - ksi));
            term2_y(abs(term2_y) < eps) = eps; % Избегаем деления на ноль
            Fn_theta_y = abs(I_y * term1_y ./ term2_y);
            Fn_theta_y = Fn_theta_y / max(Fn_theta_y); % Нормируем на максимум
            
            % 4. Трёхмерное отображение Fn(theta_x, theta_y) с учётом всех элементов решётки
            [theta_x_3d, theta_y_3d] = meshgrid(linspace(-pi/2, pi/2, 50), linspace(-pi/2, pi/2, 50));
            ...
            % Суммируем вклады всех элементов
            for i = 1:Nx_total
                for j = 1:Ny_total
                    A_ij = abs(app.Matrix(i, j)); % Амплитуда элемента (i,j)
                    phi_ij = angle(app.Matrix(i, j)); % Фаза элемента (i,j)
                    % Фазовый сдвиг для элемента (i,j) из-за углов theta_x, theta_y и theta_slider
                    phase = k * (x_positions(i) * cos(theta_x_3d + theta_offset - theta_slider - ksi) + ...
                                y_positions(j) * cos(theta_y_3d + theta_offset - ksi));
                    Fn_3d = Fn_3d + A_ij * exp(1i * (phi_ij + phase));
                end
            end
            Fn_3d = abs(Fn_3d); % Модуль комплексной суммы
            Fn_3d = Fn_3d / max(Fn_3d(:)); % Нормируем на максимум
            ...
            Fn_xy = Fn_xy / max(Fn_xy(:)); % Нормируем на максимум
            ...
            app.appendMessage(['Интенсивность I_x для ряда ' num2str(Nx) ': ' num2str(I_x)]);
            app.appendMessage(['Интенсивность I_y для столбца ' num2str(Ny) ': ' num2str(I_y)]);
            app.appendMessage(['Максимальное значение Fn(theta_x) после нормировки: ' num2str(max(Fn_theta_x))]);
            app.appendMessage(['Максимальное значение Fn(theta_y) после нормировки: ' num2str(max(Fn_theta_y))]);
            app.appendMessage(['Максимальное значение Fn_3d после нормировки: ' num2str(max(Fn_3d(:)))]);
        end
    ...
    methods (Access = public)
        ...
        function app = DipoleMatrixApp4
            ...
            app.DirectionButton.Position = [20 -20 90 22];
            ...
            app.DNPButton.Position = [130 -20 90 22];
