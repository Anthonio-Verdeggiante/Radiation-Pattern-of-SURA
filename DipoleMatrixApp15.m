classdef DipoleMatrixApp15 < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function matrix = initializeMatrix(app, powers, frequency, theta, phase_noise, amplitude_noise)
            ...
            k = 2 * pi / wavelength; % Волновое число
            
            % Высота диполя над проводящей плоскостью
            h = 11.950; % Высота, м, как указано
            ...
            % Координаты диполей (в плоскости xy, z = h для реальных, z = -h для зеркальных)
            [x, y] = meshgrid(1:12, 1:12);
            x = (x - 6.5) * dx_meters; % Смещение относительно центра
            y = (y - 6.5) * dx_meters;
            ...
            for j = 1:4
                ...
                % Реальные диполи (z = h)
                amp_base = sqrt(powers(1) / elements_per_section) * amplitude_factor * amplitude_variation(:, j);
                base_matrix(:, j, 1) = amp_base .* exp(1i * phase_1);
                base_matrix(:, j, 2) = amp_base .* exp(1i * phase_2);
                % Зеркальные диполи (z = -h) с инверсией фазы
                for i = 1:12
                    r_real = sqrt(x(i, j)^2 + y(i, j)^2 + h^2); % Расстояние до реального диполя
                    r_image = sqrt(x(i, j)^2 + y(i, j)^2 + (-h)^2); % Расстояние до зеркального диполя
                    phase_shift = k * (r_image - r_real); % Фазовый сдвиг из-за разницы расстояний
                    base_matrix(i, j, 1) = base_matrix(i, j, 1) + amp_base(i) * exp(1i * (phase_1(i) - pi - phase_shift)); % Инверсия фазы
                    base_matrix(i, j, 2) = base_matrix(i, j, 2) + amp_base(i) * exp(1i * (phase_2(i) - pi - phase_shift));
                end
            end
            ...
            for j = 1:4
                ...
                amp_base = sqrt(powers(2) / elements_per_section) * amplitude_factor * amplitude_variation(:, j);
                base_matrix(:, j+4, 1) = amp_base .* exp(1i * phase_1);
                base_matrix(:, j+4, 2) = amp_base .* exp(1i * phase_2);
                for i = 1:12
                    r_real = sqrt(x(i, j+4)^2 + y(i, j+4)^2 + h^2);
                    r_image = sqrt(x(i, j+4)^2 + y(i, j+4)^2 + (-h)^2);
                    phase_shift = k * (r_image - r_real);
                    base_matrix(i, j+4, 1) = base_matrix(i, j+4, 1) + amp_base(i) * exp(1i * (phase_1(i) - pi - phase_shift));
                    base_matrix(i, j+4, 2) = base_matrix(i, j+4, 2) + amp_base(i) * exp(1i * (phase_2(i) - pi - phase_shift));
                end
            end
            ...
            for j = 1:4
                ...
                amp_base = sqrt(powers(3) / elements_per_section) * amplitude_factor * amplitude_variation(:, j);
                base_matrix(:, j+8, 1) = amp_base .* exp(1i * phase_1);
                base_matrix(:, j+8, 2) = amp_base .* exp(1i * phase_2);
                for i = 1:12
                    r_real = sqrt(x(i, j+8)^2 + y(i, j+8)^2 + h^2);
                    r_image = sqrt(x(i, j+8)^2 + y(i, j+8)^2 + (-h)^2);
                    phase_shift = k * (r_image - r_real);
                    base_matrix(i, j+8, 1) = base_matrix(i, j+8, 1) + amp_base(i) * exp(1i * (phase_1(i) - pi - phase_shift));
                    base_matrix(i, j+8, 2) = base_matrix(i, j+8, 2) + amp_base(i) * exp(1i * (phase_2(i) - pi - phase_shift));
                end
            end
            
            % Присваиваем base_matrix в matrix
            matrix = base_matrix;
        end
        ...
        function [psi_base, Fn_psi, theta_x, Fn_theta_x, theta_y, Fn_theta_y, theta_x_3d, theta_y_3d, Fn_3d, theta_sph, phi_sph, Fn_sph] = computeAmplitudePattern(app, frequency)
            ...
            h = 11.950; % Высота диполя над плоскостью
            ...
            for i = 1:Nx_total
                for j = 1:Ny_total
                    for dipole_idx = 1:2
                        ...
                        % Вклад реального диполя (z = h)
                        phase_real = k * (x_positions(i) * cos(theta_x_3d + theta_offset - ksi) + y_positions(j) * cos(theta_y_3d + theta_offset - ksi) + h * cos(psi_base(1)));
                        directional_factor = cos(dipole_angle - atan2(cos(theta_y_3d), cos(theta_x_3d)));
                        Fn_3d = Fn_3d + A_ij * directional_factor .* exp(1i * (phi_ij + phase_real));
                        % Вклад зеркального диполя (z = -h) с инверсией фазы
                        phase_image = k * (x_positions(i) * cos(theta_x_3d + theta_offset - ksi) + y_positions(j) * cos(theta_y_3d + theta_offset - ksi) - h * cos(psi_base(1)));
                        Fn_3d = Fn_3d + A_ij * directional_factor .* exp(1i * (phi_ij + pi + phase_image)); % Инверсия фазы
                    end
                end
            end
            Fn_3d = abs(Fn_3d);
    ...
    methods (Access = public)
        ...
        function app = DipoleMatrixApp15
